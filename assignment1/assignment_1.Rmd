...
\begin{equation}
Pr[I_i^*=1|Z]=\mathbf{\Phi} (Z_i'\gamma)
\label{eq:select}
\end{equation}

In equation \ref{eq:select}, $I_i^*$ denotes an indicator variable which is equal to 1 if we observe the wage of an individual, $\bf{\Phi}(\cdot)$ denotes a normal CDF, and $Z_i$ denotes a vector of explanatory variables for the probability of an individual being employed or not.
