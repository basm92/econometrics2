...
equation \ref{eq:select}, \(I_i^*\) denotes an indicator variable which
...
Under our distributional assumptions, \(\bf{\Phi}(\cdot)\) denotes a
normal CDF, and Note that the model used in this situation concerns a
...
First, we estimate the model with the exclusion restriction, that is to
say, we have \(Z\) include schooling, age, age\^{}2, and married. Then,
we have \(X\) not include married.
