...
\CommentTok{# Stage 1:}
\NormalTok{probit <-}\StringTok{ }\KeywordTok{glm}\NormalTok{(I }\OperatorTok{~}\StringTok{ }\NormalTok{married, }
...
\NormalTok{Z_gamma <-}\StringTok{ }\KeywordTok{cbind}\NormalTok{(}\DecValTok{1}\NormalTok{,data}\OperatorTok{$}\NormalTok{married) }\OperatorTok{%*%}\StringTok{ }\KeywordTok{coef}\NormalTok{(probit) }
\NormalTok{inverse_mills_ratio <-}\StringTok{ }\KeywordTok{dnorm}\NormalTok{(Z_gamma, }\DataTypeTok{mean=}\DecValTok{0}\NormalTok{, }\DataTypeTok{sd=}\DecValTok{1}\NormalTok{) }\OperatorTok{/}\StringTok{ }\KeywordTok{pnorm}\NormalTok{(Z_gamma, }\DataTypeTok{mean=}\DecValTok{0}\NormalTok{, }\DataTypeTok{sd=}\DecValTok{1}\NormalTok{)}

\CommentTok{# Stage 2:}
\NormalTok{sample_selection_2sls <-}\StringTok{ }\KeywordTok{lm}\NormalTok{(logWage }\OperatorTok{~}\StringTok{ }\NormalTok{schooling }\OperatorTok{+}\StringTok{ }\NormalTok{age }\OperatorTok{+}\StringTok{ }\NormalTok{age2 }\OperatorTok{+}\StringTok{ }\NormalTok{inverse_mills_ratio,}
                                   \DataTypeTok{data=}\NormalTok{data)}

\CommentTok{## using package 'sampleSelection' to get maximum likelihood estimates:}
\NormalTok{sample_selection_ml <-}\StringTok{ }\KeywordTok{heckit}\NormalTok{(I }\OperatorTok{~}\StringTok{ }\NormalTok{married, }
...
\KeywordTok{stargazer}\NormalTok{(sample_selection_2sls, }
          \DataTypeTok{se =} \KeywordTok{starprep}\NormalTok{(sample_selection_2sls, }\DataTypeTok{se_type =} \StringTok{"HC1"}\NormalTok{),}
...
\StringTok{          and age squared, using two stage approach and maximum likelihood'}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
