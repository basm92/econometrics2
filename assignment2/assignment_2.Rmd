...
X_hat <- data.frame(aggregate(. ~ ID, py$earnings, mean))
...
earnings <- py$earnings
# freq is a data frame for the modified verbeek neijman:
freq <- table(earnings$ID) 
freq <- as.data.frame(freq)
  
freq$Freq <- ifelse(freq$Freq >= 5, freq$Freq, NA)
freq <- na.omit(freq)
...
# Balanced model:
balanced_formula <- paste0(py$y, " ~ ", paste(py$ivs2, collapse = " + "))

balanced_fixed_effects <- plm(formula = balanced_formula, 
                      data = freq, 
                      index = c("ID","TIME"),
                      model = "within")

summary(balanced_fixed_effects)
```
