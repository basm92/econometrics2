...
n <- length(p0)
df <- n - length(prob_4$coefficients)
alpha <- 0.05
...
# Fill vector iteratively and compute MDE:
counter <- 1
for (i in q){
    t_q[counter] <- qt(1-i, df)  # A check: from slides, t_0.7 = -0.525...

    mde[coutner] <- (t_95 - t_q[counter]) * sqrt( (1/(p(1-p))) * (sigma2 / n) )
    
    counter <- counter + 1
}


############### TO do: compute sigma2 = var(delta) and define p!
